...
{
    vector<vector<Movie*>::iterator> pos;
    for(vector<Movie*>::iterator itr = v.begin(); itr != v.end(); itr++)
    {
        if ((*itr)->rating() < 50)
            pos.push_back(itr);
    }
    for(vector<vector<Movie*>::iterator>::reverse_iterator itr = pos.rbegin(); itr != pos.rend(); itr++)
    {
        delete (*(*itr));
        v.erase(*itr);
    }
